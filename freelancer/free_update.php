...
// Fetch profile data
$current_uname = $_SESSION['username'];
$stmt = $db->prepare("SELECT * FROM free_user WHERE flancer_uname = ?");
...
$fields = [
    'flancer_name', 'flancer_email', 'flancer_phone',
    'flancer_qualification', 'flancer_uni', 'flancer_uname'
];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $new_password = !empty($_POST['flancer_pass']) ? trim($_POST['flancer_pass']) : null;

    // Validation
    if (empty($form['flancer_name'])) $errors[] = "Full name required";
    if (!filter_var($form['flancer_email'], FILTER_VALIDATE_EMAIL)) $errors[] = "Invalid email format";
    if (!preg_match('/^\d{10}$/', $form['flancer_phone'])) $errors[] = "Phone must be 10 digits";
    if (empty($form['flancer_qualification'])) $errors[] = "Qualification required";
    if (empty($form['flancer_uni'])) $errors[] = "University/Board required";

    // Username change check
    if ($form['flancer_uname'] !== $current_uname) {
        $check_stmt = $db->prepare("SELECT flancer_uname FROM free_user WHERE flancer_uname = ?");
        $check_stmt->bind_param("s", $form['flancer_uname']);
        $check_stmt->execute();
    ...
    if (empty($errors)) {
        ...
        if ($update_stmt->execute()) {
            ...
            $_SESSION['flancer_uname'] = $form['flancer_uname'];
...
<head>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            setupPasswordToggle('flancer_pass', 'toggle-pass');
            setupPasswordToggle('cnf_pass', 'toggle-cnf-pass');

            // Password validation
            const passwordInput = document.getElementById('flancer_pass');
            const requirements = {
            ...
            passwordInput.addEventListener('input', function() {
                ...
                document.getElementById('flancer_pass-error').hidden = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[\W_]).{6,}$/.test(this.value);
            });
            ...
            // Field validation
            const validations = {
                flancer_name: v => v.trim() !== '',
                flancer_email: v => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v),
                flancer_phone: v => /^\d{10}$/.test(v),
                flancer_qualification: v => v.trim() !== '',
                flancer_uni: v => v.trim() !== '',
                flancer_uname: v => v.trim() !== ''
            };
    ...
    <header class="w-full">
        <div class="mx-auto flex justify-between h-16 bg-white w-full max-w-full">
            ...
            <ul class="items-stretch hidden space-x-3 mr-5 md:flex">
                ...
                <li class="flex">
                    <a href="../freelancer/freelancer_logout.php"
    ...
    <div class="max-w-xl mx-auto mt-10 bg-white shadow-lg rounded-lg overflow-hidden">
        ...
        <form class="py-4 px-6" method="POST">
            ...
            <div class="mb-4">
                ...
                <input id="flancer_name" name="flancer_name" 
                    class="w-full p-2 border rounded hover:scale-105 duration-300"
                    value="<?= htmlspecialchars($profile['flancer_name']) ?>">
                <span class="text-red-500 text-sm" id="flancer_name-error" hidden>Full name required</span>
            </div>

            <div class="mb-4">
                <label class="block text-gray-700 font-bold mb-2">Email</label>
                <input id="flancer_email" name="flancer_email" type="email"
                    class="w-full p-2 border rounded hover:scale-105 duration-300"
                    value="<?= htmlspecialchars($profile['flancer_email']) ?>" readonly>
                <span class="text-red-500 text-sm" id="flancer_email-error" hidden>Invalid email format</span>
            </div>

            <div class="mb-4">
                <label class="block text-gray-700 font-bold mb-2">Phone Number</label>
                <input id="flancer_phone" name="flancer_phone"
                    class="w-full p-2 border rounded hover:scale-105 duration-300"
                    value="<?= htmlspecialchars($profile['flancer_phone']) ?>">
                <span class="text-red-500 text-sm" id="flancer_phone-error" hidden>10 digits required</span>
            </div>

            <!-- Education Information -->
            <div class="mb-4">
                <label class="block text-gray-700 font-bold mb-2">Qualification</label>
                <input id="flancer_qualification" name="flancer_qualification"
                    class="w-full p-2 border rounded hover:scale-105 duration-300"
                    value="<?= htmlspecialchars($profile['flancer_qualification']) ?>">
                <span class="text-red-500 text-sm" id="flancer_qualification-error" hidden>Qualification required</span>
            </div>

            <div class="mb-4">
                <label class="block text-gray-700 font-bold mb-2">University/Board</label>
                <input id="flancer_uni" name="flancer_uni"
                    class="w-full p-2 border rounded hover:scale-105 duration-300"
                    value="<?= htmlspecialchars($profile['flancer_uni']) ?>">
                <span class="text-red-500 text-sm" id="flancer_uni-error" hidden>University/Board required</span>
            </div>

            <!-- Account Information -->
            <div class="mb-4">
                <label class="block text-gray-700 font-bold mb-2">Username</label>
                <input id="flancer_uname" name="flancer_uname"
                    class="w-full p-2 border rounded hover:scale-105 duration-300"
                    value="<?= htmlspecialchars($profile['flancer_uname']) ?>" readonly>
                <span class="text-red-500 text-sm" id="flancer_uname-error" hidden>Username required</span>
            </div>
            ...
            <div class="mb-4">
                ...
                <div class="relative">
                    <input type="password" id="flancer_pass" name="flancer_pass" 
